...
function formatParticipants(participants) {
  return participants.edges.map((p) => {
    ...
    switch (p["__typename"]) {
      ...
      default:
        log.warn("getThreadList", "Found participant with unsupported typename. Please open an issue at https://github.com/Anupx0/Anupbot/issues\n" + JSON.stringify(p, null, 2));
        return {
          accountType: p["__typename"],
          userID: utils.formatID(p.id.toString()),
...
module.exports = function (defaultFuncs, api, ctx) {
  return function getThreadList(limit, timestamp, tags, callback) {
    ...
    defaultFuncs
      ...
      .catch((err) => {
        log.error("getThreadList", "Lỗi: getThreadList Maybe You Spam Too Much, Try Again !");
        return callback(err);
      });
