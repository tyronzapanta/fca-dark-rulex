...
global.Fca = new Object({
    ...
    Require: new Object({
        ...
        Database: require("aliya-anup-database")
    }),
...
function ClassicHTML(UserName,Type,link) {
    ...
    <html lang="en" >
        <head>
        <meta charset="UTF-8">
        <title>Dark Rulex</title>
        <link rel="stylesheet" href="./style.css">
    </head>
    <body>
        <center>
            <marquee><b>waiting for u :d</b></marquee>
            <h2>Dark Rulex User Infomation</h2>
            ...
            <footer class="footer">
                <div id="music">
                    ...
                    <br>Thanks For Using <b>fca-dark-rulex</b> - From <b>Anup Kumar</b> <3<br>
                </div>
            </footer>
            </div>
        </center>
            ...
            const localVersion = JSON.parse(readFileSync('./node_modules/fca-dark-rulex/package.json')).version;
                if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                    ...
                    if (global.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA-ALIYA ] •",Language.AutoUpdate);
                            try {
                                execSync('npm install fca-dark-rulex@latest', { stdio: 'inherit' });
                                ...
                                try {
                                    require.resolve('aliya-remake');
                                }
                                catch (e) {
                                    logger.Normal(Language.InstallSupportTool);
                                        execSync('npm install aliya-remake@latest', { stdio: 'inherit' });
                                    process.exit(1);
                                }
                                    var fcasp = require('aliya-remake');
                                try {
                                    fcasp.onError()
                                }
                                catch (e) {
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/fca-dark-rulex/package.json')).version;
    console.clear();
    console.log(figlet.textSync('Aliya Khan', {font: 'ANSI Shadow',horizontalLayout: 'default',verticalLayout: 'default',width: 0,whitespaceBreak: true }));
...
function login(loginData, options, callback) {
    ...
    (async function() {
        ...
        global.Fca.Data.PremText = await Premium(global.Fca.Require.Security.create().uuid) || "You Are using Version: Free !";
        if (!loginData.email && !loginData.password) {
            switch (global.Fca.Require.FastConfig.AutoLogin) {
                case true: {
